...
* PyPy 3 is ideal, but regular Python 3 or 2 also works
* [python-chess](https://github.com/niklasf/python-chess) (Note that since v0.24, python-chess is for Python 3 only.)

### Quick start

Install python-chess and then either run one of the chess engines with the UCI/XBoard interface (mainly meant for chess GUIs)
...
* EasyLearn (PTC): Learn factor / easy-play factor; e.g. if EasyLearn = 3, then pick a move randomly from the three best moves. An [exponential distribution](https://en.wikipedia.org/wiki/Exponential_distribution) with λ=EasyLambda/10 is used, so the best move has a much higher likelihood of being selected than the worst allowed move. So e.g. EasyLearn=2 can be used to add some randomness without weakening PTC too much.
...
### Running the engines from a chess GUI such as Cute Chess or Arena
